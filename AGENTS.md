...
just renderer-test # renderer統合テスト(IPC mock。macOS: WebKit、Windows: Chromium。Playwright browserの初回installが必要)
...
- 通知判定(`filter.rs`)・時限コンテンツの合致/通知選択(`content_filter.rs`)・重複排除(`dedup.rs`)・バックオフ(`backoff.rs`)は純粋関数/自己完結型で、オラクルの直接対象。副作用(時刻取得・HTTP・通知発火)を持ち込まない。現在時刻は引数で渡す
