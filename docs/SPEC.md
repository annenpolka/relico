...
| RND-013 | `renderer_scenario` | example-tested | 亀裂表のNODE列はnode名だけを表示し、enemy levelをLV表記で併記しない。行tooltipにもlevelを含めず、tier・node名・mission・enemy・mode・stormだけを含める。NODE列の表示幅が足りない場合はnode名を省略表示する。表示は設定・通知を変えない(renderer統合) |
