...
fn tmd_001() {
    ...
    assert_eq!(value["arbitration"][0]["temporalStatus"], "active", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (active wire)");
    assert_eq!(value["arbitrationPredictions"][0]["temporalStatus"], "upcoming", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (arbitrationPredictions wire)");
    assert_eq!(value["circuit"][0]["temporalStatus"], "upcoming", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (upcoming wire)");
    assert_eq!(value["arbitration"][0]["provenance"]["kind"], "community-schedule", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (schedule provenance)");
    assert_eq!(value["bounties"][0]["provenance"]["kind"], "community-live", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (community live provenance)");
    assert_eq!(value["circuit"][0]["provenance"]["kind"], "official-live", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (official provenance)");
    assert_eq!(
    ...
    assert_eq!(value["sources"]["deDescendia"]["freshness"], "stale", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (stale wire)");
    assert_eq!(value["sources"]["deCircuit"]["freshness"], "out-of-range", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (out-of-range wire)");
    assert_eq!(value["sources"]["browseWfBounties"]["freshness"], "unavailable", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (unavailable wire)");
    assert_eq!(value["sources"]["browseWfLocationBounties"]["freshness"], "stale", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (location freshness wire)");
    assert_eq!(value["areaObjectives"][0]["sourceId"], "browse-wf-location-bounties", "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (location source wire)");
    assert!(value.get("areaMissions").is_some(), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (camelCase areaMissions)");
    assert!(value.get("areaEnvironments").is_some(), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (camelCase areaEnvironments)");
    assert!(value.get("areaObjectives").is_some(), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (camelCase areaObjectives)");
    assert!(value.get("areaEvents").is_some(), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (camelCase areaEvents)");
    assert!(value.get("arbitrationPredictions").is_some(), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (camelCase arbitrationPredictions)");
    assert!(value.get("lastPoll").is_some(), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (camelCase lastPoll)");
    let encoded = serde_json::to_string(&value).unwrap();
    assert!(!encoded.contains("availability"), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (旧synthetic availabilityを残した)");
    assert!(!encoded.contains("netracells"), "SPEC TMD-001 違反: 時限content wireはcardごとにactive/upcomingの時間状態、official-live/community-live/community-scheduleのprovenance、物理contributor ID群を別fieldで持ち、仲裁Tier表をbrowse-wf-arbitration-tiersとして識別し、WFCD/DE/Oracle Bounty/Oracle location-bounties/仲裁のsource freshnessをfresh/stale/out-of-range/unavailableとしてcamelCaseで安定してserializeする。仲裁の将来表示はarbitrationPredictions、Areaの環境・通常依頼・objective rotation・追加依頼・eventは各々別sliceであり、旧synthetic availabilityとbackend netracells fieldは持たない (取得不能netracells fieldを残した)");
}

/// TMD-002: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う
#[test]
fn tmd_002() {
    let now = base_now();
    let schedule = format!(
    ...
    assert!(
        ...
        .is_err(),
        "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (Tier表はS/A/B/C/Dだけを受理)",
    );
    let card = timed::arbitration_card(&assets, now + Duration::seconds(1))
        .expect("対象時刻の仲裁cardを生成できること");

    assert_eq!(card.activation, Some(now), "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (activation)");
    assert_eq!(card.expiry, Some(now + Duration::hours(1)), "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (1時間expiry)");
    assert_eq!(card.temporal_status, timed::TimedTemporalStatus::Active, "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (active)");
    assert_eq!(card.provenance.kind, timed::TimedSourceKind::CommunitySchedule, "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (schedule provenance)");
    assert_eq!(card.source_id, timed::TimedSourceId::BrowseWfArbitrationSchedule, "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (source ID)");
    assert_eq!(card.source_name, "browse.wf", "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (source credit)");
    assert!(card.source_url.as_deref().is_some_and(|url| url.contains("browse.wf")), "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (source URL)");
    assert_eq!(card.stages.len(), 1, "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (stage数)");
    assert_eq!(card.stages[0].title, "Excavation", "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (mission)");
    assert_eq!(card.stages[0].node.as_deref(), Some("Cholistan (Europa)"), "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (node/惑星)");
    assert_eq!(card.stages[0].detail.as_deref(), Some("Infested"), "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (faction)");
    assert_eq!(card.stages[0].enemy_levels, vec![23, 33], "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (enemy level)");
    assert_eq!(
        card.metadata.iter().find(|item| item.key == "arbitrationTier").map(|item| item.value.as_str()),
        Some("A"),
        "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (Arbitration Goons Tier)",
    );
    ...
    assert_eq!(
        ...
        Some("F"),
        "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (未掲載nodeはF)",
    );
    for (key, expected) in [
        ...
        assert!(
            ...
            "SPEC TMD-002 違反: 有効な仲裁schedule行、browse.wfのArbitration Goons由来Tier表、Public Exportのregion・faction・辞書fixtureを結合すると、対象1時間のnode、惑星、mission、faction、enemy level、Dark Sector bonus、ノードID別Tierを持つcommunity-schedule cardになり、browse.wfをsourceとして保持する。Tier表に掲載された評価はS/A/B/C/Dだけを受理し、未掲載ノードはbrowse.wfと同じFとして扱う (Dark Sector {key}={expected})",
        );
    }
}

/// TMD-008: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない
#[test]
fn tmd_008() {
    let now = base_now();
    ...
    assert_eq!(current.temporal_status, timed::TimedTemporalStatus::Active, "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (現在slot)");
    assert_eq!(predictions.len(), timed::ARBITRATION_PREDICTION_LIMIT, "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (最大168件)");
    assert!(predictions.iter().all(|card| card.temporal_status == timed::TimedTemporalStatus::Upcoming), "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (upcoming区分)");
    assert_eq!(predictions[0].activation, Some(now + Duration::hours(1)), "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (現在直後から開始)");
    assert_eq!(
        current.metadata.iter().find(|item| item.key == "arbitrationTier").map(|item| item.value.as_str()),
        Some("S"),
        "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (現在slot Tier)",
    );
    assert_eq!(
        predictions[0].metadata.iter().find(|item| item.key == "arbitrationTier").map(|item| item.value.as_str()),
        Some("B"),
        "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (予測slot Tier)",
    );
    assert_eq!(
        predictions.last().and_then(|card| card.activation),
        Some(now + Duration::hours(timed::ARBITRATION_PREDICTION_LIMIT as i64)),
        "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (activation昇順)",
    );
    ...
    assert!(tail.is_empty(), "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (schedule末尾を循環・補間)");

    let mut snapshot = timed::TimedContentSnapshot::default();
    snapshot.arbitration = vec![current];
    snapshot.arbitration_predictions = predictions;
    assert_eq!(snapshot.all_cards().filter(|card| card.kind == "arbitration").count(), 1, "SPEC TMD-008 違反: 仲裁は現在slotをactiveな通知対象sliceに1件だけ保持し、その直後から最大168件の連続slotをactivation昇順のupcomingな表示専用arbitrationPredictions sliceへ分離する。予測はschedule末尾を越えて循環・補間せず、現在slotと将来予測の双方を同じbrowse.wf schedule/Tier表/Public Exportから解決して各cardへTierを保持する。表示専用予測はcontentRulesの通知候補を列挙するallCardsへ混入させない (予測を通知候補へ混入)");
}
